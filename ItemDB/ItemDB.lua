...
function ItemDB:TestAdvancedFilter(itemLink)
	wipe(advanced_filter_itemstats)
...
function ItemDB:AdvancedFilter_Update()
	...
	for i = 1, #frames_advancedfilterlist do
		...
		if advanced_filter[filter] == true then
			frames_advancedfilterlist[i]:SetText("|TInterface\\RaidFrame\\ReadyCheck-Ready:20:20|t|cff00ff00".._G[filter])
		elseif advanced_filter[filter] == false then
			frames_advancedfilterlist[i]:SetText("|TInterface\\RaidFrame\\ReadyCheck-NotReady:20:20|t|cffff0000".._G[filter])
		else
			frames_advancedfilterlist[i]:SetText("|TInterface\\RaidFrame\\ReadyCheck-Waiting:20:20|t|cff999999".._G[filter])
		end
	end
end
...
local it_del, it_new
do
	local it_cache = setmetatable({},{__mode = 'k'})
	it_del = function(t)
			it_cache[t] = true
		end
	it_new = function()
			local t = next(it_cache) or {}
			it_cache[t] = nil
			return t
		end
end
...
function ItemDB:ItemList_Create()
	-- clear the item table, recycle the subtables
	for k, v in pairs(itemlist_filtered) do
		itemlist_filtered[k] = nil
		it_del(v)
	end

	local FilterName = string_lower(ItemDB_Browser_Filter_Name:GetText())

	-- check if they are looking up an item/spell by ID
	if string_match(FilterName, "^item:(%-?%d+)") then
		local id = string_match(FilterName, "^item:(%-?%d+)")
		...
		local itemtable = it_new()
		itemtable.id		= id
		itemtable.type		= "item"
		itemtable.name		= itemName			or "item:"..id
		itemtable.rarity	= itemRarity or -1
		itemtable.itemlevel	= itemLevel
		itemtable.minlevel	= itemMinLevel
		itemtable.stack		= itemStackCount
		itemtable.value		= itemSellPrice
		table_insert(itemlist_filtered, itemtable)
		ItemDB_Browser_ItemCount:SetText("lookup by id")
		return
	elseif string_match(FilterName, "^spell:(%-?%d+)") then
		local id = string_match(FilterName, "^spell:(%-?%d+)")
		local spellName, spellRank, spellTexture = GetSpellInfo(id)
		local itemtable = it_new()
		itemtable.id		= id
		itemtable.type		= "spell"
		itemtable.name		= spellName			or "spell:"..id
		itemtable.rarity	= nil
		itemtable.itemlevel	= nil
		itemtable.minlevel	= nil
		itemtable.stack		= nil
		itemtable.value		= nil
		table_insert(itemlist_filtered, itemtable)
		ItemDB_Browser_ItemCount:SetText("lookup by id")
	...
	local itemlist, itemlist_size, prefiltered = itemprovider:GetItemList(filterset)
	local filter = filters[filterset.name]
	if not prefiltered then
		filter:PrepareFilter(filterset)
	end
	
	local FilterName = string_lower(ItemDB_Browser_Filter_Name:GetText())
	local FilterMinLevel = tonumber(ItemDB_Browser_Filter_MinLevel:GetText())
	local FilterMaxLevel = tonumber(ItemDB_Browser_Filter_MaxLevel:GetText())
	for id, data in pairs(itemlist) do
		...
		if id_type == "item" then
			...
			if	filter_rarity[itemRarity] and (itemRarity < 0 or (		-- rarity / uncached
					true and (not FilterMinLevel or itemMinLevel >= FilterMinLevel) and (not FilterMaxLevel or itemMinLevel <= FilterMaxLevel) and (string_match(string_lower(itemName), FilterName))
			  and	(prefiltered or filter:TestItem(id, itemName, itemLink, itemRarity, itemLevel, itemMinLevel, itemType, itemSubType, itemStackCount, itemEquipLoc, itemTexture))
			...
			local spellName, spellRank, spellTexture = GetSpellInfo(id)
			if filter_rarity[-2] and (not spellName or string_match(string_lower(spellName), FilterName)) and (prefiltered or filter:TestSpell(id, spellName, spellRank, spellTexture)) then
	...
	ItemDB_Browser_ItemCount:SetFormattedText("%d / %d items", #itemlist_filtered, itemlist_size)
end
...
Script_ItemButton_OnEnter = function(self)
	...
	if item.type == "spell" then
		...
		if item.rarity < 0 and GetItemInfo(item.id) then -- item new in cache since last search
			local itemName, itemLink, itemRarity, itemLevel, itemMinLevel, itemType, itemSubType, itemStackCount, itemEquipLoc, itemTexture = GetItemInfo(item.id)
			item.name		= itemName
...
Script_ResetButton_OnClick = function(self)
	ItemDB_Browser_ItemCount:SetText("")
